...
- Bound the buffer that assembles a Hyprland event line, and drop an over-long one rather than accumulating it. Event lines carry window titles, which the application owning the window chooses and can make any length. An event here is only ever a wake-up, so a dropped line costs at most the poll interval the event socket exists to avoid.
- Hold a stored desktop launcher to the same id pattern the entry scan produces, rather than accepting any string ending in `.desktop`. Nothing downstream interpolates an id unquoted, so this closes no exploit; it keeps a path or a line break from being saved as one.
- Keep newly matched windows provisional while their compositor-visible surface state is still changing, and keep long-lived updater, splash, or class-only surfaces provisional for the full launch deadline so a slot rebinds to the application's real window.
- Defer temporary floating until each group workspace is actively being finalized, restore saved floating modes even when activation fails or a client disappears during cleanup, and continue finalizing independent targets before reporting the failure.
