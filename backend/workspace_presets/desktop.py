...
class DesktopEntry:
    ...
    Written out rather than generated by ``dataclass`` because importing that
    module pulls in ``inspect`` and ``ast``, which cost more than every other
    import in this package combined on a command that only reads a JSON file.
    """

    __slots__ = (
        "desktop_id", "name", "startup_class", "executable",
        "terminal", "no_display", "path", "webapp_url",
    )
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, DesktopEntry):
    ...
    def __repr__(self) -> str:
        values = ", ".join(f"{name}={getattr(self, name)!r}" for name in self.__slots__)
        return f"DesktopEntry({values})"
...
class OmarchyPanelPlugin:
    __slots__ = ("plugin_id", "name", "path")

    def __init__(self, plugin_id: str, name: str, path: str):
        self.plugin_id = plugin_id
        self.name = name
        self.path = path

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, OmarchyPanelPlugin):
            return NotImplemented
        return all(
            getattr(self, name) == getattr(other, name) for name in self.__slots__
        )
    ...
    def __repr__(self) -> str:
        return (
            f"OmarchyPanelPlugin(plugin_id={self.plugin_id!r}, "
...
def _read_desktop_group(path: Path) -> dict[str, str] | None:
    """Read the ``[Desktop Entry]`` group, stopping at the next group header.

    A desktop entry is a flat key=value list and only six of its keys are read
    ...
    for data_dir in _data_dirs():
        ...
        for path in sorted(applications.rglob("*.desktop")):
            ...
            if desktop_id in entries or not SAFE_DESKTOP_ID.fullmatch(desktop_id):
                continue
            section = _read_desktop_group(path)
            if section is None:
                continue
            if section.get("Type", "Application") != "Application":
                continue
            if section.get("Hidden", "").casefold() in TRUE_VALUES:
                continue
            ...
            entries[desktop_id] = DesktopEntry(
                ...
                terminal=section.get("Terminal", "").casefold() in TRUE_VALUES,
                no_display=section.get("NoDisplay", "").casefold() in TRUE_VALUES,
                path=str(path),
...
def scan_process_table(proc_root: Path = Path("/proc")) -> dict[int, dict] | None:
    """Read every process's tree and TTY fields in one pass.

    Capturing a workspace with several terminals repeated this whole scan once
    per terminal - about 19 ms across 600 processes each time - so a caller
    handling more than one window builds the table once and passes it down.
    """
...
def _foreground_terminal_process(
    terminal_pid: int,
    proc_root: Path,
    ...
    if records is None:
        records = scan_process_table(proc_root)
        if records is None:
            return None
    ...
    *,
    ...
    try:
        ...
        if "-e" in argv:
            ...
            foreground = _foreground_terminal_process(
                terminal_pid, proc_root, records
            )
